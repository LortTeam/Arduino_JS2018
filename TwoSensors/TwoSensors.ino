...
void setup() {
  ...
  Timer1.initialize(300000);  // Запускаем таймер с перидом 300мс для оцифровки КГР
...
void loop() {
  ...
  Serial.write("A0");  // Отправка имени порта, к которому подключен КГР
  Serial.write(map(analogRead(A0),0,1024,0,255)); // Отправка значения сигнала КГР 
}
